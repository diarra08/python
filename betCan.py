...
#################################################################################################
# Fonctions qui servent à charger les bases de données sur le fichier texte dans un dictionnaire#
#################################################################################################
def Load_db1(): 
...
def Load_db2():
    label_titre.configure(text='\n    Pari Football Spécial Can 2019     \n')
    Data_base = "".join(open("bendo.txt","r"))
    Data_base = Data_base.replace("\n",",")
    Data_base = Data_base.split(",")
...
def Load_db6():
    label_titre.configure(text='')
    Data_base = "".join(open("DONG.txt","r"))
    Data_base = Data_base.replace("\n",",")
    Data_base = Data_base.split(",")
...
def intro():   
	...
	label.configure(text="Combien de questions voulez vous pour parier ? \n 1 question vaut 10 000fr. \nLaisser un espace entre les chiffres exemple 1 1",fg="black")
	labelVIES.configure(text='Vous disposez de 5 chances')
	labelScore.configure(text='Votre coin : 0 bt')
	valider= Button(root,text="PARIER",command=nb_questions)
...
def quizz():
	...
	if len(deja)< nb_Questions and vies>0:
		...
		labelVIES.configure(text='Vous avez fini votre pari')
		Compteur.configure(text="")
		label.configure(text="Vous avez eu %d bonnes réponses sur %d vos différents pari !\nPour un nouveau pari veillez cliquer sur pari football CAN 2019" % (bonnes,nbQuestAffich),fg="black")
		...
		valider.configure(text='¨PARIER',bg="green",state=DISABLED)
		Indice.configure(state=DISABLED)
...
def check():
	...
	if  T1 == T2 :
		...
		SS=0
		label.configure(text="  FÉLICITATION!  ",fg="green")
		if score > 0:
			labelScore.configure(text='Votre coin:%d bt'% score)
		else:
			score=0
			labelScore.configure(text='Votre coin: 0 bt')
		valider.configure(text="Pari Suivant",command=quizz)
	else :
		SS=0 
		...
		label.configure(text="Ah tu as tapé potooooo!, c'est %s" % reponses[j],fg="red")
		valider.configure(text="Pari Suivant",command=quizz)
		vies=vies-1  
		labelVIES.configure(text='Vous avez encore %d chance'% vies)
		if score > 0:
			labelScore.configure(text='Votre coin:%d bt'% score)
		else:
			score =0
...
##################################################################
def ouvrir():
    ofi = open("regles.txt","r")
    t= ofi.read()
    reg=Tk()
    reg.title('Regles du pari')
...
root=Tk()
root.title("betCan2019")
root.geometry("900x275")
root.configure(bg="white")


label_titre = Label(root,text="\n     PARIER ET GAGNER DE L'ARGENT EN TOUTE SIMPLICITÉ...     \n",bg="blue",fg="white")
...
bb = Button(root,text="Regles du pari",command=ouvrir)
bb.grid(column=3,row=0,sticky="W")

     
DB2 = Button(root,text=" Pari Football CAN 2019 ",command=Load_db2)    
